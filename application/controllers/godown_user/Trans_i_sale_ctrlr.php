...
class Trans_i_sale_ctrlr extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Interstate Sale';
        $data['company_name'] = $this->general->get_firm();
        $purchases = $this->general->get_max_data('interstate_sale', 'invoice_number');
        if ($purchases) {
        ...
        $data['main_content'] = 'godown_user/trans_i_sale';
        $this->load->view('templates/standard', $data);
    }

    public function save_sale() {
        $data['title'] = 'Interstate Sale';
        $invo = $this->general->get_max_data('interstate_sale', 'invoice_number');
        if ($invo) {
            foreach ($invo as $inv)
                ;
        ...
        $purchase_id = $this->general->save('interstate_sale', $purchase);
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            $this->general->save('interstate_sold_inventory', $data);
        }
        ...
        $account_data1 = array(
            ...
            'description' => 'Interstate Sale - Invoice No: ' . $this->input->post('reference_no'),
        ...
        $pdf_data = array('sale_data' => $purchase,
            ...
            'total' => $total);
        $html = $this->load->view('print_pdf_interstate', $pdf_data, true);
        $print['Attachment'] = 0;
        pdf_create($html, 'i_sale_' . $purchase_id, $print);
    }

    public function update_sale() {
        $data['title'] = 'Interstate Sale Update';
...
//            'sale_type' => $this->input->post('sale_type'),
            ...
            'status' => 3,
            'sale_total' => $this->input->post('net_total'));
        $this->general->update('interstate_sale', $sale, $sale_id);

        $old_sold_inventory_datas = $this->general->get_data_wer('interstate_sold_inventory', 'sale_id', $sale_id);
        ...
        $this->general->delete_wer('interstate_sold_inventory', 'sale_id', $sale_id);
        ...
        $account_data1 = array(
            ...
            'account_group' => 'Sales Accounts',
            'amount' => $this->input->post('net_total'),
            'description' => 'Interstate Sale - Invoice No: ' . $this->input->post('invoice_no'),
            'txn_date' => date("Y-m-d", strtotime($this->input->post('sale_date'))),
            'txn_type' => 'payment',
            'from_or_to' => 'Sale',
            'sale_id' => $sale_id
        ...
        $html = $this->load->view('print_pdf_interstate', $pdf_data, true);
        $print['Attachment'] = 0;
        pdf_create($html, 'i_sale_' . $sale_id, $print);
    }
    ...
    public function delete_sale() {
        ...
        $current_sale = $this->general->get_data_wer('interstate_sale', 'id', $sale_id);
        if ($invoice != $current_sale[0]->invoice_number) {
            $update_data1 = array('status' => 1);
            $this->general->update('interstate_sale', $update_data1, $sale_id);
        } else {
            $this->general->delete('interstate_sale', $sale_id);
        }
        //check

        $sold_inventorys = $this->general->get_data_wer('interstate_sold_inventory', 'sale_id', $sale_id);
        ...
        redirect('reports/interstate-sale', 'refresh');
    }

    public function edit_sale($invoice_id) {
        $data['title'] = 'Edit Interstate Sale';
        $data['company_name'] = $this->general->get_firm();
        $sale_details = $this->general->get_data_wer('interstate_sale', 'invoice_number', $invoice_id);
        ...
        $data['main_content'] = 'godown_user/trans_i_sale_edit';
        $this->load->view('templates/standard', $data);
    }

    public function edit_sale_data() {
        $sale_id = $this->input->get('sale_id');
        $single_sales = $this->general->get_data_wer('interstate_sold_inventory', 'sale_id', $sale_id);
        $si_no = 1;
