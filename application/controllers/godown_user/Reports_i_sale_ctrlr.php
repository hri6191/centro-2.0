...
class Reports_i_sale_ctrlr extends CI_Controller {

    public function index() {
        $data['title'] = 'Interstate Sale Report';
        $data['company_name'] = $this->general->get_firm();
        $data['main_content'] = 'godown_user/reports_i_sale';
        $this->load->view('templates/standard', $data);
    }

    public function sale_data() {
        $sale_datas = $this->general->get_data_wer('interstate_sale', 'created_by', $this->session->userdata('id'));
    ...
    public function single_sale($invoice_id) {
        ...
        $data['main_content'] = 'godown_user/reports_i_sale_single';
        $this->load->view('templates/standard', $data);
    }
