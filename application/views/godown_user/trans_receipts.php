<div class="">
    <div class="large-12 small-12 columns" id="navigate">
        <ul class="breadcrumbs">
            ...
            <li class="current"><a >Receipt</a></li>
        </ul>
    </div>
</div>
<div class="large-12 small-12 columns" id="navigate">
    <h4>Receipt</h4>
    <div class="row">
        <form action="<?php echo $this->config->base_url(); ?>transaction/receipts/edit-mode" name="for_edit" id="for_edit" method="post">
            <input type="text" id="edit_id" name="edit_id" tabindex="1" placeholder="For Edit, Search Here...">
        </form>
    </div>
</div>
<?php if (isset($edit_mode)) { ?>
    <form data-abide name="form_material" action="<?php echo $this->config->base_url(); ?>transaction/receipts/edit" method="post">
        <input type="hidden" name="id" value="<?php echo $edit_mode[0]->id; ?>">
    <?php } else { ?>
        <form data-abide name="form_firm" action="<?php echo $this->config->base_url(); ?>transaction/receipts/add" method="post">
        <?php } ?>
        <form data-abide>
            <div class="row">
                <div class="large-4 columns name-field">
                    <label>Account
                        <input type="text" name="account_name" <?php if(isset($edit_mode)) echo 'readonly'; ?> id="account_name" class="focus" value="<?php echo isset($edit_mode[0]->account_name) ? $edit_mode[0]->account_name : '' ?>" tabindex="1" required >
                    </label>
                </div>
                <div class="large-4 columns name-field">
                    <label>Account Group
            ...
            <div class="row">
                ...
                <div class="large-4 columns name-field">
                    <label>To Account
                        <input type="text" <?php if(isset($edit_mode)) echo 'readonly'; ?> name="from_account_name" id="from_account_name" value="<?php echo isset($edit_mode[0]->from_or_to) ? $edit_mode[0]->from_or_to : 'cash_book' ?>" tabindex="1" required >
                    </label>
                </div>
                <div class="large-4 columns name-field">
                    <label>Date
            ...
            <div class="row">
                ...
                <?php if(isset($edit_mode)) { ?> <a href="<?php echo base_url().'transaction/receipts/delete/'.$edit_mode[0]->id; ?>"><?php } ?>
        ...
        <script>
            $(document).ready(function() {
                $("#edit_id").autocomplete({
                    url: '<?php echo base_url(); ?>Search/account_receipt_list?output=json',
