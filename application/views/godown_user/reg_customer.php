...
<div class="large-12 small-12 columns" id="navigate">
    ...
    <div class="row">
        <form action="<?php echo $this->config->base_url(); ?>registration/customer/edit-mode" name="for_edit" id="for_edit" method="post">
            <input type="text" id="edit_id" name="edit_id" tabindex="1" placeholder="For Edit, Search Here..." role="submit">
        </form>
    </div>
</div>
<?php if (isset($edit_mode)) { ?>
    <form data-abide name="form_customer" action="<?php echo $this->config->base_url(); ?>registration/customer/edit" method="post">
        <input type="hidden" name="id" value="<?php echo $edit_mode[0]->id; ?>">
    <?php } else { ?>
        <form data-abide name="form_customer" action="<?php echo $this->config->base_url(); ?>registration/customer/add" method="post">
        <?php } ?>
        <div class="row">
            <div class="large-4 columns name-field">
                <label>Customer Name *&nbsp;&nbsp;<span class="" id="status"></span>
                    <input type="text" class="focus" name="customer_name" id="customer_name" value="<?php echo isset($edit_mode[0]->vendor_name) ? $edit_mode[0]->vendor_name : '' ?>" tabindex="1" required parameter="<?php echo isset($edit_mode[0]->vendor_name)?$edit_mode[0]->vendor_name:null ?>" >
                    <input type="hidden" name="old_customer_name" id="old_customer_name" value="<?php echo isset($edit_mode[0]->vendor_name) ? $edit_mode[0]->vendor_name : '' ?>" tabindex="1" >
                </label>
